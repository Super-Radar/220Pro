...
ADC 数据必须先按实际 RadarTools 配置整理为“采样点 × chirp × 帧”的复数数据立方体。下面代码从这个数据立方体开始，完成距离维 FFT、多普勒维 FFT 和一阶 MTI，并返回统一的距离轴、速度轴及处理前后幅度。窗函数使用显式公式生成，不依赖额外 MATLAB 工具箱。

```matlab
function out = process_entryway_adc(adcCube, fs, slope, fc, chirpPeriod)
% adcCube: [numSamples, numChirps, numFrames] complex array
% fs: ADC sampling rate (Hz), slope: chirp slope (Hz/s)
% fc: center frequency (Hz), chirpPeriod: chirp repetition period (s)

c = 299792458;
[numSamples, numChirps, ~] = size(adcCube);

% Hann windows written explicitly to avoid toolbox dependencies.
n = (0:numSamples-1).';
rangeWindow = 0.5 - 0.5*cos(2*pi*n/max(numSamples-1, 1));
m = 0:numChirps-1;
dopplerWindow = 0.5 - 0.5*cos(2*pi*m/max(numChirps-1, 1));

rangeCube = fft(adcCube .* reshape(rangeWindow, [], 1, 1), [], 1);
numRangeBins = floor(numSamples/2);
rangeCube = rangeCube(1:numRangeBins, :, :);

rdBefore = fftshift(fft( ...
    rangeCube .* reshape(dopplerWindow, 1, [], 1), [], 2), 2);

% First-order cancellation along slow time suppresses stationary clutter.
mtiCube = rangeCube(:, 2:end, :) - rangeCube(:, 1:end-1, :);
numMtiChirps = size(mtiCube, 2);
mMti = 0:numMtiChirps-1;
mtiWindow = 0.5 - 0.5*cos(2*pi*mMti/max(numMtiChirps-1, 1));
rdAfter = fftshift(fft( ...
    mtiCube .* reshape(mtiWindow, 1, [], 1), [], 2), 2);

beatFrequency = (0:numRangeBins-1) * fs / numSamples;
out.range_m = c * beatFrequency / (2*slope);

lambda = c / fc;
kBefore = -floor(numChirps/2):ceil(numChirps/2)-1;
kAfter = -floor(numMtiChirps/2):ceil(numMtiChirps/2)-1;
out.velocity_before_mps = kBefore * lambda / ...
    (2*numChirps*chirpPeriod);
out.velocity_after_mps = kAfter * lambda / ...
    (2*numMtiChirps*chirpPeriod);

out.rd_before_db = 20*log10(abs(rdBefore) + eps);
out.rd_after_db = 20*log10(abs(rdAfter) + eps);
end
```

`fs`、`slope`、`fc` 和 `chirpPeriod` 必须来自本次采集配置，不能使用示例值替代。MTI 会削弱零多普勒附近的能量，因此人员静止后的结果必须同时查看 `rd_before_db` 和 `rd_after_db`；不能把 MTI 后目标减弱直接解释为人员离开。

### 6.3 现场记录格式

现场记录分为场地表 `site.csv` 和连续采集表 `captures.csv`。两张表通过 `environment_id` 关联；同一批数据只填写一次场地尺寸和软件版本，每次连续采集则在 `captures.csv` 中单独占一行。场地表的字段和空白示例如下：

```csv
environment_id,capture_date,location_type,radar_model,radar_alias,radar_height_m,radar_yaw_deg,radar_pitch_deg,near_distance_m,far_distance_m,corridor_width_m,doorway_width_m,mount_description,background_reflectors,radartools_version,notes
E01,,indoor-entryway,CTSAI-A100,,,,,,,,,,,,
```

采集表保留场景编号、重复序号、匿名参与者编号、动作标签、参考距离、数据路径和隐私检查结果。下面每个场景只展示第一行；正式采集时应将 `repeat_index` 分别写为 1、2、3，形成三次独立记录，不覆盖失败或中断的采集。

```csv
record_id,environment_id,scene_id,repeat_index,participant_id,target_count,action_label,start_time,duration_s,reference_distance_m,stop_duration_s,data_types,radar_config_file,data_file,ground_truth_file,privacy_checked,notes
S0-R01,E01,S0,1,,0,empty,,,,,,,,,,
S1-R01,E01,S1,1,P01,1,approach,,,,,,,,,,
S2-R01,E01,S2,1,P01,1,recede,,,,,,,,,,
S3-R01,E01,S3,1,P01,1,stationary,,,,,,,,,,
S4-R01,E01,S4,1,P01,1,stop_continue,,,,,,,,,,
S5-R01,E01,S5,1,P01,1,lateral_pass,,,,,,,,,,
S6-R01,E01,S6,1,P01,1,turn_back,,,,,,,,,,
```

可以使用下面的 Python 代码生成完整的 S0～S6、每组重复三次的 `captures.csv`。输出文件使用独占创建模式 `"x"`，当同名文件已经存在时程序会停止，因此不会覆盖先前记录。
...
FIELDS = [
    "record_id", "environment_id", "scene_id", "repeat_index",
    "participant_id", "target_count", "action_label", "start_time",
    "duration_s", "reference_distance_m", "stop_duration_s", "data_types",
    "radar_config_file", "data_file", "ground_truth_file",
    "privacy_checked", "notes",
]

def create_capture_table(path, environment_id="E01"):
    rows = []
    for scene_id, (action, target_count) in SCENARIOS.items():
        for repeat_index in range(1, 4):
            row = dict.fromkeys(FIELDS, "")
            row.update({
                "record_id": f"{scene_id}-R{repeat_index:02d}",
...
create_capture_table("captures.csv")
```

`environment_id` 只使用字母、数字、点、下划线和连字符，并以字母或数字开头。`capture_date` 使用 `YYYY-MM-DD`，`start_time` 使用带时区的 ISO 8601 时间。距离和尺寸统一使用米，角度使用度，时长使用秒；未知值保持为空，不用 `0` 代替。`participant_id` 只能填写匿名编号，`data_types` 可填写 `point_cloud`、`target`、`can`、`adc`，同次采集包含多种数据时用分号分隔。数据和配置文件路径相对于数据集根目录书写并统一使用 `/`。只有确认文件不含未授权人脸、姓名、门牌等信息后，才能将 `privacy_checked` 填为 `true`；采集中断、动作偏离、临时遮挡和雷达移动等情况写入 `notes`。

此前采集的小推车、行李箱或纸箱数据属于不同实验，不能直接改写标签后作为人员出入口 S0～S6 数据。每次人员实验应在仓库外建立新的数据目录，并在开始处理前检查：`record_id` 是否唯一、S0～S6 是否各有三次记录、采集表中的 `environment_id` 是否能在场地表中找到、未复测字段是否仍为空。上述检查只能说明记录结构完整，不能证明填写的实测数值正确。
