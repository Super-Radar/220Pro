...
![S0～S6 受控实验场景矩阵](./images/indoor-entryway-scenario-matrix.png)

图 2 将七组动作放在同一符号坐标系中对照。红色和蓝色箭头分别表示 $\dot r<0$ 与 $\dot r>0$，紫色表示静止或横向运动；图中不包含实测视场或性能边界。图片由[独立生成脚本](./scripts/generate_entryway_scenario_matrix.py)绘制，并保留 [SVG 矢量版本](./images/indoor-entryway-scenario-matrix.svg)。
...
![S1～S6 理想距离—时间曲线](./images/indoor-entryway-ideal-range-time.png)

图 3 给出各动作在理想条件下应满足的距离—时间关系。靠近和远离对应相反的 $\dot r$ 符号，静止段斜率近似为零，横向经过产生以最近点为中心的 U 形距离，折返则在 $t_r$ 处改变斜率符号。坐标经过归一化，曲线不是实测数据。生成脚本与 [SVG 矢量图](./images/indoor-entryway-ideal-range-time.svg)分别位于[此处](./scripts/generate_entryway_ideal_range_time.py)和图片目录。
...
现场记录可复制[场地参数模板](./templates/indoor-entryway-site.csv)和[采集文件清单模板](./templates/indoor-entryway-captures.csv)后填写。两张表通过 `environment_id` 关联，字段含义、单位、动作标签和隐私要求见[模板说明](./templates/README.md)。仓库中的模板保持为空白状态，实际采集记录应与数据文件一同保存，不应覆盖模板原件。
...
![室内墙面一次反射与镜像目标几何](./images/indoor-entryway-multipath-geometry.png)

图 4 用镜像法解释一种理想墙面反射。若反射往返路径长度 $L_r$ 大于直达往返路径 $L_d$，则表观距离偏差为 $\Delta R=(L_r-L_d)/2>0$，可能表现为距离偏大的墙后点或贴墙假轨迹。实际门口还可能存在地面、门框和多次反射，因此该图只用于解释现象，不用于直接校正数据。[生成脚本](./scripts/generate_entryway_multipath_geometry.py)和 [SVG 矢量图](./images/indoor-entryway-multipath-geometry.svg)均随文档保留。
