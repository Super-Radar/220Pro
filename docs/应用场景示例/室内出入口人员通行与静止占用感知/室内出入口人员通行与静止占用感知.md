...
仓库根目录下可执行以下命令，重新生成本文的四张科学示意图并检查 PNG、SVG 和文档内本地链接：

```bash
python "docs/应用场景示例/室内出入口人员通行与静止占用感知/scripts/validate_entryway_figures.py"
```

命令以 `PASS  indoor-entryway figures and links are reproducible` 结束时，表示绘图脚本均已正常运行、图像文件可解析，且正文引用的本地资源均存在。该检查只验证图形工程的完整性，不代表雷达实测数据或算法性能通过验证。
