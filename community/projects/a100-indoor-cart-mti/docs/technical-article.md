...
项目使用自己的 `src/load_a100_adc.m` 跳过三个头字段，并覆盖错误头部、短文件和非零尾部等失败路径。官方示例文件保持不变，相关修改不属于本项目提交范围。
...
MTI 前后二维图使用同一个 MTI 前峰值作为 0 dB 参考，因此移动目标的一阶 MTI 衰减不会在后图中被重新归一化成 0 dB。
...
静止目标与空场在每个 Rx 内共用幅度参考，四通道距离谱存在差异，但它们同时包含墙面、地面、多径、通道增益和安装几何。不同 Rx 之间不共用增益标定，不能只凭某个 bin 增强就断言它对应塑料目标的精确距离。
...
对于运动场景，算法不读取人工场景方向，而是分别建立靠近和远离假设。候选限定在 0.8–4.5 m 近场并满足 SNR 门限；超过 2 s 的时间空隙、超过 0.75 m 的距离跳变或明显逆向跳变会切断分段。只有整体距离斜率与径向速度符号一致的分段才可参与自动选择，最终构成保守检测包络：
...
`data/manifest.csv` 对 31 个正式文件记录相对路径、字节数和 SHA-256。`run_all.m` 会在生成任何结果前自动验证全部条目；文件缺失、大小变化或同长度内容篡改都会停止运行。
