...
The committed reference figures, CSV files and per-profile logs were generated by
the NumPy validator from the repository's public captures. The same source commit
was also executed natively with MATLAB R2026a on a GitHub-hosted Ubuntu runner:
`main('near')` returned 32 rows and `main('far')` returned 3 rows. Raw Doppler
bins matched NumPy exactly; maximum range and power differences were below
`5e-14 m` and `5e-13 dB`. See
`validator/results/matlab-r2026a-validation.txt`. No radar hardware test is claimed.
